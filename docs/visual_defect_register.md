...
| EXACT_JLC_SYMBOL_FIDELITY | Pending Web GPT review | User clarified the requirement is exact JLC symbol reuse, not an approximate JLC-style redraw. | The generator now deep-clones each component group from `jlc_schematic_original.svg`; every symbol fidelity entry records source/final element counts, path counts, geometry hash, stroke hash, and allowed translate-only transform. Audit result: 21/21 PASS, 0 blockers. |
| EXACT_REVIEW_CROP_ALIGNMENT | Pending Web GPT review | Previous crops mixed module content, especially sensor/UART, heater/power, and power-only areas. | Review crop boxes were tightened so `sensor_uart_area` focuses on R2/XS1/XS4, `heater_power_area` focuses on R4/R5/VT1/XS2/XS5, and `power_area` focuses on XS3/A1/C3/C4. |
