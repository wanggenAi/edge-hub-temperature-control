      ...
      <div className="mx-auto flex max-w-[1600px] gap-4 p-4">
        <aside className={`${collapsed ? "w-16" : "w-56"} shrink-0 rounded-xl border border-line/80 bg-panel/90 p-2.5 shadow-panel transition-all`}>
          ...
          <nav className="space-y-1">
            ...
            {hasRole("admin") && (
              <NavItem to="/ops" icon={<Settings2 className="h-4 w-4" />} label="DevOps Console" collapsed={collapsed} />
            )}
            {hasRole("admin") && (
              <NavItem to="/storage-rules" icon={<Database className="h-4 w-4" />} label="Storage Rules" collapsed={collapsed} />
            )}
            {hasRole("admin") && (
