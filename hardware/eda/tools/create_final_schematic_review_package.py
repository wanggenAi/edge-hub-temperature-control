...
def crop_metadata() -> dict[str, dict[str, Any]]:
    return {
        ...
        "heater_power_area": {
            "related_refs": ["R4", "R5", "VT1", "XS2", "XS5"],
            "related_nets": ["GATE", "GATE_R", "HEAT+", "HEAT-", "+12V", "GND"],
            "purpose": "Heater driver and safety/output terminal review.",
...
def save_crops(png_path: Path, viewbox: dict[str, float], regions: dict[str, Any], embed: dict[str, float]) -> list[dict[str, Any]]:
    ...
    crops: list[tuple[str, dict[str, float], float | tuple[float, float, float, float]]] = [
        ...
        ("dd1_area", sub_box(embed, 0.24, 0.18, 0.34, 0.50), 22.0),
        ("reset_boot_led_area", sub_box(embed, 0.00, 0.34, 0.58, 0.62), 22.0),
        ("sensor_uart_area", sub_box(embed, 0.49, 0.04, 0.38, 0.44), 22.0),
        ("heater_power_area", sub_box(embed, 0.56, 0.44, 0.42, 0.36), 22.0),
        ("power_area", sub_box(embed, 0.55, 0.78, 0.40, 0.22), 22.0),
