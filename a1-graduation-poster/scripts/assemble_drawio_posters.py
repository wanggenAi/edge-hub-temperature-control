...
Back: blank A1 side, preserving only the aa.drawio title block/table.
"""
...
def light_pill(builder: DiagramBuilder, x: float, y: float, w: float, text: str, color: str) -> None:
    builder.rect(x, y, w, 50, fill="#ffffff", stroke=color, radius=1, width=2)
    builder.text(text, x, y + 5, w, 36, size=23, color="#122333", weight="1", align="center")
...
def build_front() -> None:
    b = DiagramBuilder("Front A1 Poster", background="#eef6fb")
    b.rect(0, 0, PAGE_W, PAGE_H, fill="#eef6fb", stroke="none", radius=0)
    b.rect(0, 0, PAGE_W, 170, fill="#dfeff6", stroke="none", radius=0)

    # Top band.
    b.text("EdgeHub Temperature Control", 80, 34, 1520, 58, size=48, color="#122333", weight="1")
    b.text("Intelligent Edge-Hub Platform for Thermal PID Optimization", 82, 94, 1500, 36, size=25, color="#425a6d")
    tag_x = 1750
    for label, width, color in [
        ...
        light_pill(b, tag_x, 66, width, label, color)
        tag_x += width + 16

    # Large hero card: use the enclosure as the main visual impact.
    b.rect(70, 210, 1660, 1645, fill="#ffffff", stroke="#9bb4c2", radius=1, width=2)
    b.text("Edge Device / Thermal Enclosure", 110, 242, 920, 42, size=34, color="#122333", weight="1")
    b.text("3D thermal insulation enclosure and edge temperature-control prototype", 112, 288, 1120, 30, size=21, color="#425a6d")
    b.image(ASSET_DIR / "enclosure-hero.png", 130, 330, 1540, 1450)
    callouts = [
        (150, 475, "Insulated Chamber", "#5aa9e6", 670, 625),
        (1240, 510, "Safety Barrier", "#2ad4a0", 1165, 690),
        (142, 1120, "DS18B20 Probe", "#ef8354", 640, 1216),
        (1185, 1230, "Heater Zone", "#f0b84a", 1052, 1048),
        (1065, 1658, "Electronics Bay", "#9f7aea", 980, 1505),
    ]
    ...
    # Data-Hub panel.
    b.rect(1770, 210, 1460, 480, fill="#ffffff", stroke="#9bb4c2", radius=1, width=2)
    b.text("Data-Hub Runtime Cluster", 1810, 242, 720, 38, size=31, color="#122333", weight="1")
    b.text("MQTT ingress, Java Reactor ingestion, buffering, persistence, and runtime observability", 1812, 284, 1120, 28, size=19, color="#425a6d")
    b.image(ASSET_DIR / "data-hub.svg", 1800, 328, 770, 345)
    b.text("Runtime Contract", 2635, 344, 430, 34, size=24, color="#122333", weight="1")
    for i, (topic, color) in enumerate(
        ...
        b.rect(2638, 395 + i * 62, 520, 42, fill="#f9fcff", stroke=color, radius=1, width=2)
        b.text(topic, 2655, 403 + i * 62, 485, 24, size=18, color="#23384a")

    # HMI screenshots.
    b.rect(1770, 730, 1460, 720, fill="#ffffff", stroke="#9bb4c2", radius=1, width=2)
    b.text("HMI Layer", 1810, 762, 480, 38, size=31, color="#122333", weight="1")
    b.text("Real FastAPI + React screenshots with seeded telemetry data", 1812, 804, 920, 28, size=19, color="#425a6d")
    b.rect(1810, 850, 650, 36, fill="#eaf5fb", stroke="#5aa9e6", radius=1, width=2)
    b.text("Device Detail + Parameter Apply", 1810, 855, 650, 24, size=21, color="#122333", weight="1", align="center")
    b.rect(1810, 895, 650, 486, fill="#030b12", stroke="#5aa9e6", radius=1, width=4)
    b.image(ASSET_DIR / "hmi-device-detail-crop.png", 1822, 907, 626, 462)
    b.rect(2510, 850, 650, 36, fill="#eafbf5", stroke="#2ad4a0", radius=1, width=2)
    b.text("AI Validation", 2510, 855, 650, 24, size=21, color="#122333", weight="1", align="center")
    b.rect(2510, 895, 650, 218, fill="#030b12", stroke="#2ad4a0", radius=1, width=4)
    b.image(ASSET_DIR / "hmi-ai-validation-crop.png", 2528, 906, 614, 196)
    b.rect(2510, 1150, 650, 36, fill="#fff7e6", stroke="#f0b84a", radius=1, width=2)
    b.text("Ops Console", 2510, 1155, 650, 24, size=21, color="#122333", weight="1", align="center")
    b.rect(2510, 1195, 650, 186, fill="#030b12", stroke="#f0b84a", radius=1, width=4)
    b.image(ASSET_DIR / "hmi-ops-console-crop.png", 2530, 1199, 610, 178)

    # AI-assisted decision panel.
    b.rect(1770, 1495, 1460, 360, fill="#ffffff", stroke="#9bb4c2", radius=1, width=2)
    b.text("AI-Assisted Decision", 1810, 1523, 650, 38, size=31, color="#122333", weight="1")
    b.text("features, recommendation, preview, operator apply, and ACK validation", 1812, 1564, 1000, 28, size=19, color="#425a6d")
    b.image(ASSET_DIR / "ai-decision.svg", 1810, 1612, 500, 244)
    ...
    b.rect(70, 1900, 3160, 350, fill="#ffffff", stroke="#9bb4c2", radius=1, width=2)
    b.text("Key Contributions", 110, 1932, 780, 42, size=34, color="#122333", weight="1")
    items = [
    ...
    for number, title, subtitle, color in items:
        contribution_light(b, x, 2034, 585, number, title, subtitle, color)
        x += 610

    b.write(POSTER_ROOT / "front-a1-poster.drawio")


def title_block_cells() -> list[ET.Element]:
    source_root = ET.parse(AA_DRAWIO).getroot()
    model = source_root.find("./diagram/mxGraphModel")
    if model is None:
        raise RuntimeError("Cannot locate mxGraphModel in aa.drawio")
    root = model.find("root")
    if root is None:
        raise RuntimeError("Cannot locate root in aa.drawio")
    out: list[ET.Element] = []
    for cell in root.findall("mxCell"):
        if cell.attrib.get("id") == "pFFQBGnBG81xobuCz_b_-1" or cell.attrib.get("parent") == "pFFQBGnBG81xobuCz_b_-1":
            cloned = copy.deepcopy(cell)
            if cloned.attrib.get("id") == "pFFQBGnBG81xobuCz_b_-36":
                cloned.set("value", '<font style="font-size: 12px;">EdgeHub Temperature Control<br>Graduation Project Poster</font>')
            out.append(cloned)
    return out


def table_row(builder: DiagramBuilder, x: float, y: float, widths: list[float], values: list[str], *, header: bool = False) -> None:
    color = "#e8f2f7" if header else "#ffffff"
    stroke = "#6b7d8b"
    text_color = "#122333" if header else "#23384a"
    cx = x
    for width, value in zip(widths, values):
        builder.rect(cx, y, width, 54, fill=color, stroke=stroke, radius=0, width=1.4)
        builder.text(escape(value), cx + 12, y + 8, width - 24, 32, size=20 if header else 18, color=text_color, weight="1" if header else "0")
        cx += width
...
def build_back() -> None:
    ...
    # Preserve only the formal title block/table from aa.drawio. The rest of
    # the back side stays blank for the required duplex-print title-block side.
    for cell in title_block_cells():
        b.root.append(cell)
